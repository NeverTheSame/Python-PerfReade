...
Usage:
    ...
    docker run -v /path/to/logs/:logs.zip feedthemachine/perf-reader:master-9e6fe7b logs.zip

Examples:
    To process an (unzipped) support log:
        {sys.argv[0].split("/")[-1]} ~/path/to/Office365Logs.zip
        """, file=sys.stdout)
...
def process_proxy_files():
    ...
    for actual_file_name in os.listdir(os.path.abspath(path_to_extracted_folder)):
        if variables["proxy_file_name"] in actual_file_name:
